...
//!
//! * `noob-vst-webgui-framework-net`: a single-threaded tokio runtime running axum. Accepts
...
//! * `noob-vst-webgui-framework-pump`: a plain thread that sleeps until the audio thread
...
//! | `GET /noob-vst-webgui-framework/<file>` | the browser library baked into the binary ([`CLIENT_ASSETS`]) |
...
use crate::bridge::{Message, NoobVstWebguiFramework, ParamChange, Shared};
...
/// ```
/// use std::time::Duration;
/// use noob_vst_webgui_framework::{ServerConfig, WakeMode};
///
...
/// The browser-side library, baked in and served under `/noob-vst-webgui-framework/`, so a
/// page can `import '/noob-vst-webgui-framework/noob-vst-webgui-framework.js'` with no bundler. The files are
/// the ones in `web/` at the repository root at build time.
pub const CLIENT_ASSETS: &[(&str, &[u8])] = &[
    (
        "noob-vst-webgui-framework.js",
        include_bytes!("../web/noob-vst-webgui-framework.js"),
    ),
    (
        "components/knob.js",
...
/// Body of the `noob-vst-webgui-framework-pump` thread. One cycle:
///
...
<code>/ws</code> and the client library is served at <code>/noob-vst-webgui-framework/noob-vst-webgui-framework.js</code>.</p>";

/// The fallback route: `/noob-vst-webgui-framework/<file>` from [`CLIENT_ASSETS`], everything
...
async fn static_handler(State(st): State<Arc<AppState>>, uri: Uri) -> Response {
    let path = uri.path();
    if let Some(rest) = path.strip_prefix("/noob-vst-webgui-framework/") {
...
pub fn serve(bridge: &NoobVstWebguiFramework, cfg: ServerConfig) -> io::Result<ServerHandle> {
    ...
    let pump = {
        ...
        thread::Builder::new()
            .name("noob-vst-webgui-framework-pump".into())
    ...
    let net = thread::Builder::new()
        .name("noob-vst-webgui-framework-net".into())
        .spawn(move || {
