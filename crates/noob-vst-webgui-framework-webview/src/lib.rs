...
//!
//! A noob-vst-webgui-framework plug-in serves its interface as a web page from a local server
//! (`noob_vst_webgui_framework::serve`). This crate puts that page *inside* the editor
...
//! updates from the OS. The nih-plug adapter (`noob-vst-webgui-framework-nih`) is the main
...
//! through `noob-vst-webgui-framework`.
//!
//! # The UI-thread timer
//!
//! VST3 and CLAP want parameter edits (begin / perform / end) to reach the
//! host from the main (GUI) thread. Edits arrive on noob-vst-webgui-framework's network thread,
...
pub struct WebViewOptions {
    /// The page to load; for a noob-vst-webgui-framework plug-in the URL of its own server,
    ...
    /// define `window.noobVstWebguiFrameworkHost = {...}` so the page can tell it is
