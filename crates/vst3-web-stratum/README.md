...
The heart of [vst3-web-stratum](../../README.md): a low-latency local
...
| host / GUI | `Vst3WebStratum` | push host changes, drain edits and messages, own the UI store; short uncontended mutexes |
| `vst3-web-stratum-pump` | internal | wakes on publish, encodes each frame once, hands it to every client without blocking |
| `vst3-web-stratum-net` | internal | single-threaded tokio + axum; HTTP, WebSocket, decoding of inbound frames |
...
Measured on the examples (release build, 386 parameters): edit-to-echo
...
The page side is the `@elyerinfox/vst3-web-stratum` library (`web/` inside this crate, at the repository
root), which the server also serves under `/vst3-web-stratum/`:
...
| module | contents |
|---|---|
| `bridge` | `Vst3WebStratum`, `AudioHandle`, `Vst3WebStratumBuilder`, the shared state, the UI store, edit / event / message queues |
...
Run `cargo doc --open -p vst3-web-stratum` for the API reference. The
...
* [`docs/`](../../docs/): architecture, guides and the example walkthroughs.
* `vst3-web-stratum-nih`: the nih-plug editor adapter (embedded web view, gesture
