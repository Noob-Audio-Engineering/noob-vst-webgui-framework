...
//!   the built-in client library is served under `/vst3-web-stratum/` and path
...
use vst3_web_stratum::wire::{self, EditPhase, Frame, PARAM_FLAG_ECHO, PARAM_FLAG_HOST};
use vst3_web_stratum::{ParamSpec, ServerConfig, StreamKind, StreamSpec, Vst3WebStratum, serve};
...
async fn full_round_trip() {
    ...
    tcp.write_all(
        b"GET /vst3-web-stratum/vst3-web-stratum.js HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n",
    )
    .await
    .unwrap();
    ...
    assert!(resp.contains("export class Vst3WebStratumClient"));
...
async fn events_flow_both_ways() {
    use vst3_web_stratum::wire::{EventsWriter, UiEvent, event_kind};
    let s = bridge();
...
async fn sticky_streams_are_replayed_to_late_clients() {
    let s = Vst3WebStratum::builder("sticky")
...
async fn instance_endpoint_and_discovery_file() {
    use vst3_web_stratum::discovery;
    let s = bridge();
    let server = serve(&s, ServerConfig::default()).unwrap();
...
async fn instances_are_scoped_to_the_plugin_name() {
    ...
    let other = Vst3WebStratum::builder("itest-other")
