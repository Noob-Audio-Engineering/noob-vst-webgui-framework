/**
 * vst3-web-stratum.js — browser client for the vst3-web-stratum bridge.
 *
 ...
 *
 * One `Vst3WebStratumClient` owns one WebSocket to one plug-in instance. Through it
 * the page gets:
 *
 ...
 *
 * 1. `new Vst3WebStratumClient(url, opts)` opens the socket at once. `url`
 ...
 *    populated. `Vst3WebStratumClient.connect()` resolves here.
...
/**
 * Event kinds (mirrors `vst3_web_stratum::wire::event_kind`). `>= 0x80` is plugin-defined.
 *
...
export class Param {
   ...
   * pages.
   * @param {Vst3WebStratumClient} client
 ...
 * `'manifest'` when usable) or await {@link Vst3WebStratumClient.connect}.
 *
 * Events (subscribe with {@link Vst3WebStratumClient#on}):
 *
 ...
 */
export class Vst3WebStratumClient {
  /**
   ...
   * @returns {Promise<Vst3WebStratumClient>}
   */
  static connect(url, opts = {}) {
    return new Promise((resolve, reject) => {
      const c = new Vst3WebStratumClient(url, opts);
      let timer = null;
  ...
  /**
   * Send one event; see {@link Vst3WebStratumClient#sendEvents}.
   * @param {UiEvent} e
   * @returns {void}
   */
  sendEvent(e) {
   ...
   * @param {Float32Array} snapshot From {@link Vst3WebStratumClient#snapshot}.
   * @param {{ emit?: boolean }} [opts]
   * @returns {number} How many parameters changed.
   */
 ...
 * @param {string} id  Element id used for de-duplication (`'vst3-web-stratum-knob'`).
...
export default Vst3WebStratumClient;
