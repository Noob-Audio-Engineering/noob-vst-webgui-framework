 ...
 *
 * Styling: none. Root class `vst3-web-stratum-toggle` plus the variant
...
<template>
  <button
    v-if="variant === 'button'"
    type="button"
    class="vst3-web-stratum-toggle button"
  ...
  <div v-else-if="variant === 'rocker'" class="vst3-web-stratum-toggle rocker" :class="{ 'is-on': p.on, vertical, disabled }" role="radiogroup">
  ...
  <label v-else class="vst3-web-stratum-toggle switch" :class="{ 'is-on': p.on, disabled }">
