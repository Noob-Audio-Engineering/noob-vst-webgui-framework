 ...
 * the knob. Themed through CSS variables (`--vst3-web-stratum-accent`, `--vst3-web-stratum-text`).
 *
 * Usage:
 *
 ...
 *   `--vst3-web-stratum-accent` variable (fallback `#5ac8fa`) when null.
 ...
 *
 * CSS variables honoured: `--vst3-web-stratum-accent` (arc, focus ring, field
 * border), `--vst3-web-stratum-text` (pointer line, value), `--vst3-web-stratum-text-dim`
 * (label), `--vst3-web-stratum-bg` (text field background).
 */
...
const stroke = computed(() => props.color || 'var(--vst3-web-stratum-accent, #5ac8fa)');
const el = ref(null);
...
<template>
  ...
  >
    <svg :width="size" :height="size" viewBox="0 0 100 100" class="sk-svg">
      ...
      <line x1="50" y1="26" x2="50" y2="35" stroke="var(--vst3-web-stratum-text, #e2e8f0)" stroke-width="4" stroke-linecap="round" :transform="`rotate(${angle.toFixed(2)} 50 50)`" />
    </svg>
    <input
      v-if="editing"
      ref="input"
...
.sk:focus-visible {
  box-shadow: 0 0 0 2px var(--vst3-web-stratum-accent, #5ac8fa);
}
.sk-svg {
  display: block;
  overflow: visible;
...
.sk-input {
  ...
  background: var(--vst3-web-stratum-bg, #0d1016);
  color: inherit;
  border: 1px solid var(--vst3-web-stratum-accent, #5ac8fa);
  border-radius: 4px;
  padding: 2px 4px;
  outline: none;
