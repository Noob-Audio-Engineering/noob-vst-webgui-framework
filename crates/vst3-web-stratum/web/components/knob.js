/**
 * Knob — an SVG rotary control bound to a vst3-web-stratum Param.
 *
 ...
 * ## Styling
 *
 * CSS variables: `--vst3-web-stratum-text` (indicator, value text), `--vst3-web-stratum-accent`
 * (value arc, focus ring; `opts.color` overrides it per knob),
 * `--vst3-web-stratum-track` (background arc), `--vst3-web-stratum-knob-body` (disc fill).
 * Classes on the root `.vst3-web-stratum-knob`: `svg`, `.track`, `.value`, `.ind`,
 * `.body`, `.val` (value text), `.lbl` (label).
 */
import { injectStyle } from '../vst3-web-stratum.js';

const CSS = `
.vst3-web-stratum-knob{display:inline-flex;flex-direction:column;align-items:center;gap:2px;
  user-select:none;-webkit-user-select:none;touch-action:none;cursor:ns-resize;outline:none;
  font:12px/1.2 system-ui,-apple-system,Segoe UI,sans-serif;color:var(--vst3-web-stratum-text,#e6e6e6)}
.vst3-web-stratum-knob:focus-visible svg{filter:drop-shadow(0 0 3px var(--vst3-web-stratum-accent,#5ac8fa))}
.vst3-web-stratum-knob svg{display:block;overflow:visible}
.vst3-web-stratum-knob .track{fill:none;stroke:var(--vst3-web-stratum-track,rgba(255,255,255,.14));stroke-linecap:round}
.vst3-web-stratum-knob .value{fill:none;stroke:var(--vst3-web-stratum-accent,#5ac8fa);stroke-linecap:round}
.vst3-web-stratum-knob .ind{stroke:var(--vst3-web-stratum-text,#e6e6e6);stroke-linecap:round}
.vst3-web-stratum-knob .body{fill:var(--vst3-web-stratum-knob-body,rgba(255,255,255,.06))}
.vst3-web-stratum-knob .val{font-variant-numeric:tabular-nums;opacity:.95}
.vst3-web-stratum-knob .lbl{opacity:.6;font-size:11px;letter-spacing:.02em;text-transform:uppercase}
`;
...
export class Knob {
   ...
   * @param {import('../vst3-web-stratum.js').Param} param The parameter to show and edit.
   * @param {object} [opts]
   ...
   * @param {string} [opts.color] Accent colour for this knob only (sets `--vst3-web-stratum-accent` on the root).
  ...
  constructor(container, param, opts = {}) {
    injectStyle('vst3-web-stratum-knob-css', CSS);
    this.param = param;
    this.opts = {
      size: 72,
      label: param.name,
    ...
    el.className = 'vst3-web-stratum-knob';
    el.tabIndex = 0;
    el.setAttribute('role', 'slider');
    el.setAttribute('aria-label', this.opts.label);
    el.setAttribute('aria-valuemin', '0');
    el.setAttribute('aria-valuemax', '1');
    if (this.opts.color) el.style.setProperty('--vst3-web-stratum-accent', this.opts.color);
