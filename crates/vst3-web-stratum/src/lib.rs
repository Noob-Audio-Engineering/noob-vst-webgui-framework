//! vst3-web-stratum: a low-latency local WebSocket bridge between an audio
...
//! | host / GUI (plug-in side) | [`Vst3WebStratum`] | pushes host parameter changes, drains edits and messages, owns the UI store |
//! | `vst3-web-stratum-pump` (spawned by `serve`) | internal | wakes when something was published, encodes each frame once, hands it to every client |
//! | `vst3-web-stratum-net` (spawned by `serve`) | internal | single-threaded tokio + axum: HTTP, WebSocket, decoding of inbound frames |
//!
...
//! same edit is then queued for the host ([`Vst3WebStratum::drain_edits`]) or handed
...
//! [`Vst3WebStratumBuilder::build`]. The [`Vst3WebStratum`] handle, by contrast, takes
...
//!
//! ```no_run
//! use vst3_web_stratum::{Vst3WebStratum, ParamSpec, StreamSpec, StreamKind, ServerConfig};
//!
...
//! let server = vst3_web_stratum::serve(&bridge, ServerConfig::default().prefer_port(4242)).unwrap();
//! println!("{}", server.url());
//!
...
//! # Module map
//!
//! * [`bridge`]: [`Vst3WebStratum`] / [`AudioHandle`] and the state they share; the
...
//!   standalone hosts.
//!
//! The matching browser library is `@elyerinfox/vst3-web-stratum` (`crates/vst3-web-stratum/web/vst3-web-stratum.js` in the
//! repository, also served by the server under `/vst3-web-stratum/`); the nih-plug
//! adapter is the `vst3-web-stratum-nih` crate.
...
pub use bridge::{
    AudioHandle, EditEvent, EditHook, Message, ParamChange, StoreHook, Vst3WebStratum,
