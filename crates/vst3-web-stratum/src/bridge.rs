...
//!  host / GUI thread
//!  ─────────────────
//!  Vst3WebStratum::set_param_norm ─► ParamStore + to_ui queue ─► ParamValues frames
//!  Vst3WebStratum::drain_edits    ◄── to_host queue            ◄── ParamEdit frames
//!  Vst3WebStratum::poll_message   ◄── inbound_json             ◄── {"t":"msg"} text
//!  Vst3WebStratum::send_json      ─► outbound_json             ─► {"t":"msg"} text
//!  Vst3WebStratum::store_*        ◄► store (JSON object)       ◄► store.* text
//! ```
//!
//! # Building
//!
//! [`Vst3WebStratum::builder`] collects [`ParamSpec`]s and [`StreamSpec`]s; their
//! order fixes the `u16` indices used everywhere else. [`Vst3WebStratumBuilder::build`]
//! allocates every queue and mailbox once and renders the manifest JSON, so
//! nothing after that allocates on the audio path.
//!
//! # Handles
//!
//! * [`Vst3WebStratum`]: cheap to clone, for the host / GUI side. Takes short,
//!   uncontended mutexes.
//! * [`AudioHandle`]: exactly one per bridge ([`Vst3WebStratum::take_audio`]),
...
//! |---|---|---|---|
//! | `to_ui` | plug-in → clients (parameter changes) | `ui_queue`, default 4096 | the change is dropped and counted ([`Vst3WebStratum::dropped_ui_changes`]); clients resync from the store on their next full snapshot |
...
/// A parameter change heading to the UI. Produced by the `Vst3WebStratum` and
...
/// for [`Vst3WebStratum::drain_edits`]. The lowest-latency route to a host; the
...
/// Declare parameters and streams, then [`build`](Vst3WebStratumBuilder::build).
///
/// ```
/// use vst3_web_stratum::{ParamSpec, Vst3WebStratum, StreamKind, StreamSpec};
///
/// let bridge = Vst3WebStratum::builder("demo")
...
/// ```
pub struct Vst3WebStratumBuilder {
    name: String,
    meta: Value,
...
impl Vst3WebStratumBuilder {
    /// Start a bridge called `name` (the manifest `name`, the discovery
    /// record, the default port-probing base). Same as [`Vst3WebStratum::builder`].
    pub fn new(name: impl Into<String>) -> Self {
        Vst3WebStratumBuilder {
            name: name.into(),
            meta: Value::Null,
            params: Vec::new(),
            streams: Vec::new(),
    ...
    pub fn build(self) -> Vst3WebStratum {
...
/// there.
#[derive(Clone)]
pub struct Vst3WebStratum {
    pub(crate) shared: Arc<Shared>,
}

impl Vst3WebStratum {
    /// Start declaring a bridge called `name`.
    pub fn builder(name: impl Into<String>) -> Vst3WebStratumBuilder {
        Vst3WebStratumBuilder::new(name)
    }
...
///
/// Exactly one exists per bridge ([`Vst3WebStratum::take_audio`]). Typical use in
/// a process callback:
///
/// ```no_run
/// # use vst3_web_stratum::{AudioHandle, UiEvent, event_kind};
...
mod tests {
    ...
    fn bridge() -> Vst3WebStratum {
        Vst3WebStratum::builder("test")
            .param(
