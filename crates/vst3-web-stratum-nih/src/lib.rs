...
//!
//! `vst3-web-stratum` gives a plug-in a parameter mirror, telemetry streams and a
//! local WebSocket server; `vst3-web-stratum-webview` embeds the OS web view in a host
//! window. This crate joins the two to nih-plug:
//!
//! * [`mirror_params`] turns a nih-plug [`Params`] object into vst3-web-stratum
...
//!   knowing its formula.
//! * [`Vst3WebStratumEditor`] owns the bridge and the server, mirrors host changes
...
//! ```
//!
//! On the page, `@elyerinfox/vst3-web-stratum` connects to `/ws` on the same origin, receives
//! the manifest with every parameter, and its knobs send gestures back; see
//! the repository's `docs/GETTING-STARTED.md`.
//!
//! # Lifecycle
//!
//! 1. **Construction** ([`Vst3WebStratumEditor::new`] /
//!    [`with_builder`](Vst3WebStratumEditor::with_builder)): the
...
//!    ([`Vst3WebStratumEditor::ensure_server`]), the mirror is refreshed from the
...
//! 6. **Plug-in dropped**: the last [`Vst3WebStratum`] handle goes away, the server
...
//! | audio thread | host | `process`: reads parameters through nih-plug as usual, publishes streams with [`AudioHandle`](vst3_web_stratum::AudioHandle) (wait-free) |
//! | `vst3-web-stratum-pump` | vst3-web-stratum | fans parameter changes and stream frames out to clients |
//! | `vst3-web-stratum-net` | vst3-web-stratum | the WebSocket server; receives edits, messages and store writes |
//!
...
//! ([`vst3_web_stratum_webview::UiTimer`], every [`EditorConfig::forward_interval`])
...
//! # Parameter mirroring
//!
//! [`mirror_params`] walks `Params::param_map()` in order, so vst3-web-stratum's
...
//! range is taken, so several instances (of this plug-in or of other vst3-web-stratum
...
//! instance also writes a discovery record (`vst3_web_stratum::discovery`) and
...
use nih_plug::prelude::*;
use vst3_web_stratum::bridge::EditEvent;
use vst3_web_stratum::{
...
///   which becomes the parameter's [`Taper`](vst3_web_stratum::Taper) on the
...
///
/// Called by [`Vst3WebStratumEditor::with_builder`]; public so a custom editor or a
...
pub struct EditorConfig {
    ...
    /// (only `/ws` and the built-in `/vst3-web-stratum/*` client library are served;
    ...
    /// the real lower bound; see [`vst3_web_stratum_webview::UiTimer`]).
...
///
/// Owns the bridge ([`Vst3WebStratum`]), the mirrored parameter pointers, the
...
pub struct Vst3WebStratumEditor {
    bridge: Vst3WebStratum,
    /// One pointer per mirrored parameter, in vst3-web-stratum index order.
    ptrs: Vec<ParamPtr>,
    width: AtomicU32,
    height: AtomicU32,
    min_size: (u32, u32),
...
unsafe impl Send for Vst3WebStratumEditor {}
unsafe impl Sync for Vst3WebStratumEditor {}

impl Vst3WebStratumEditor {
    ...
    pub fn new(
        ...
        cfg: EditorConfig,
    ) -> (Arc<Vst3WebStratumEditor>, Vst3WebStratum) {
    ...
    ///
    /// `customize` receives the [`Vst3WebStratumBuilder`](vst3_web_stratum::Vst3WebStratumBuilder)
    ...
    pub fn with_builder(
        ...
        customize: impl FnOnce(
            vst3_web_stratum::Vst3WebStratumBuilder,
        ) -> vst3_web_stratum::Vst3WebStratumBuilder,
    ) -> (Arc<Vst3WebStratumEditor>, Vst3WebStratum) {
        ...
        let editor = Arc::new(Vst3WebStratumEditor {
            bridge: bridge.clone(),
            ptrs,
    ...
    pub fn bridge(&self) -> &Vst3WebStratum {
        &self.bridge
    }
    ...
    pub fn ensure_server(&self) -> Option<String> {
        ...
        if g.is_none() {
            ...
            match vst3_web_stratum::serve(&self.bridge, cfg) {
                Ok(s) => *g = Some(s),
                Err(e) => {
...
/// [`Vst3WebStratumEditor`]. Get one from [`Vst3WebStratumEditor::handle`].
pub struct EditorHandle(Arc<Vst3WebStratumEditor>);
...
impl Editor for EditorHandle {
    ...
    /// The size the host should give the window: see [`Vst3WebStratumEditor::size`].
...
/// [`Vst3WebStratum::store_json`]) inside the plug-in's state. Put one in your
...
struct StoreSlotInner {
    bridge: Option<Vst3WebStratum>,
    pending: Option<String>,
}

impl StoreSlot {
    /// The key used inside the plug-in's persistent fields.
    pub const KEY: &'static str = "vst3_web_stratum_ui_store";
    ...
    pub fn attach(&self, bridge: &Vst3WebStratum) {
